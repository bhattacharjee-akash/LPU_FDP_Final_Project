...
def run_agentic_workflow(syllabus_id: int, user_id: str, provider: str, model_name: str, temp: float):
    ...
    try:
        ...
        if provider == "gemini":
            if model_name in ["gemini-1.5-flash", "gemini-2.5-flash", "gemini-2.0-flash", "gemini-flash-latest"]:
                model_name = "gemini-2.0-flash"
            elif model_name in ["gemini-1.5-pro", "gemini-2.5-pro", "gemini-2.0-pro", "gemini-pro-latest"]:
                model_name = "gemini-2.0-pro"
        elif provider == "groq":
            if model_name in ["mixtral-8x7b-32768", "llama3-70b-8192", "llama-3.3-70b-versatile"]:
                model_name = "llama-3.3-70b-versatile"
            elif model_name in ["llama-3.1-8b-instant"]:
        ...
        if provider == "gemini" and gemini_key and not (gemini_key.startswith("AIzaSy") or gemini_key.startswith("AQ.")):
            raise ValueError(
                f"Your GEMINI_API_KEY format is invalid. Google Gemini keys must start with 'AIzaSy' or 'AQ.' "
                f"(yours starts with '{gemini_key[:8] if gemini_key else 'None'}...'). Please obtain a valid API key "
                f"and update it in your Render settings."
            )
